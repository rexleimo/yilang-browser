...
- 多标签页管理，支持无痕浏览
- 会话恢复：冷启动回到上次退出前的标签页；标签概览一键恢复最近关闭
- 地址栏建议：本地来源（书签/历史/标签/近期搜索）+ 联网搜索词建议
- 站点权限（Android）：相机/麦克风/定位请求弹窗，会话内按站点记忆
- 桌面版网站开关（桌面 UA + 宽视口）
- 页面内查找（关键词高亮）

### 面板工具
- 阅读模式：提取正文与图片，过滤干扰元素（阅读清单条目优先用净化副本打开）
- 保存页面截图（Android 原生长截图拼接）
- 分享页面
- 翻译为中文
- 收藏网页 / 加入阅读清单（已读/未读、未读筛选、一键转书签）
- 历史记录 / 阅读清单列表
- 下载中心：Android 系统下载器托管，进度/取消/重试/打开文件
- 清除浏览数据：历史、Cookie、缓存、近期搜索、离线副本
- 书签导入导出（Netscape HTML，与主流浏览器互通）
- 浏览器设置

### 设计亮点
- 动态主题色 —— 状态栏颜色跟随网页 `<meta name="theme-color">` 自动变化
- 阅读清单支持离线归档
- 首页背景自定义 —— 预设渐变 / 相册图片（自动暗色遮罩），设置页「外观」或 Home 设置弹层均可更换
- 标签页预览卡片（带截图 + 圆角裁切）
- 暗色模式支持
